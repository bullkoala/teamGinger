class FollowersController < ApplicationController
  ...
  def index
    if params[:search]
      @followers = Follower.search(params[:search]).order('title DESC')
    else 
      @followers = Follower.all.order('title DESC')
    end
  end
  ...
  private
    ...
    def follower_params
      params.require(:follower).permit(:typeID, :strength, :chi, :personalHonor, :goldCost, :honorRequirement, :keywords, :description, :focus)
    end
end
