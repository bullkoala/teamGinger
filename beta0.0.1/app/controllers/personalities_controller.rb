class PersonalitiesController < ApplicationController
  ...
  def index
    if params[:search]
      @personalities = Personality.search(params[:search]).order('title DESC')
    else
      @personalities = Personality.all.order('title DESC')
    end
  end
  ...
  private
    ...
    def personality_params
      params.require(:personality).permit(:typeID, :strength, :chi, :goldCost, :familyHonorRequirement, :personalHonor, :keywords, :description, :clan)
    end
end
